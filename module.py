...
def find_poss_line(N, hint):
    ...
    blank_list = [0]*(n_hint+1)

    all_b_line = []
    poss_line = []
    # 가능한 경우 탐색
    for i in range((d+1)**(n_hint+1)):
        if i >= 1:
            blank_list[0] += 1
        for j in range(n_hint):
            if blank_list[j] == d+1:
                blank_list[j] = 0
                blank_list[j+1] += 1
        if blank_list[-1] == d and blank_list[0] == 1:
            break
        # print(blank_list)
        ...
        if sum(blank_list) == d:
            ...
            for j in range(len(blank_list)):
                if j == 0 or j == len(blank_list)-1:
                    temp.append(blank_list[j])
                else:
                    temp.append(blank_list[j]+1)
            # print(temp)
